...
harness, and the fused MuJoCo-Warp self-play match.
"""
...
def step_reward(dealt: float = 0.0, taken: float = 0.0, closing: float = 0.0,
    ...
    Backend-agnostic scalars flow through identical arithmetic."""
    return (DAMAGE_MAX * (dealt - taken)
            + AGGRESSION_MAX * (closing - fleeing)
            + CONTROL_MAX * control)


def step_reward_array(dealt, taken, closing, fleeing, control, *, xp=None):
    """Clamped array twin for NumPy or Torch tensors."""
    if xp is None:
        try:
            import torch
            if any(isinstance(value, torch.Tensor)
                   for value in (dealt, taken, closing, fleeing, control)):
                xp = torch
            else:
                import numpy as xp
        except ImportError:
