...
void Drv8316r::update(double t_s, const DriverInputs& in,
                      const std::array<double, 3>& phase_currents_a,
                      double pvdd_v, double die_temp_c) {
  dc_cal_ = in.dc_cal;

  // EN/nSLEEP sequencing: ready after the wake window (no SPI config step).
  if (in.en_gate && !prev_en_gate_) en_rise_time_ = t_s;
  if (!in.en_gate) {
    en_rise_time_ = -1.0;
    ready_ = false;
    otsd_latched_ = false;       // wake clears latched faults
  ...
  // VM UVLO with hysteresis (rising 4.4 / falling 4.2).
  ...
  // Overtemperature: OTW (warning -> noctw pulse) and OTS (latched shutdown).
  ...
  const bool enabled =
      in.en_gate && ready_ && !pvdd_uv_ && !otsd_latched_;
  ...
  for (int k = 0; k < 3; ++k) {
    ...
    // Dead-time floor + complementary handshake.
    if (want_gh && !gh_[k] &&
    ...
    // Integrated-FET overcurrent: a fixed current limit (not a VDS sense).
    if (enabled && (gh_[k] || gl_[k]) &&
        std::abs(phase_currents_a[k]) > config_.ocp_level_a) {
      ++oc_events_;
      noctw_until_ = t_s + config_.noctw_pulse_s;
      if (config_.ocp_latch) {
        leg_latched_[k] = true;
