...
#   PHASE=N [MJX_PY=...] bash scripts/fw_snapshot.sh
set -u
PHASE="${PHASE:?set PHASE=N}"
MJX_PY="${MJX_PY:-$HOME/mjx-venv/bin/python}"
ROOT="$(cd "$(dirname "$0")/.." && pwd)"; cd "$ROOT"; export MJX_PY
LP="import sys; sys.path.insert(0,'sim/robot'); from arena import _ledger"
VCMD=$("$MJX_PY" -c "$LP; print(_ledger.verify_cmd($PHASE))")
...
if ( eval "$VCMD" ); then            # subshell: a `cd` in the verify cmd must NOT leak to the tar
  ...
  if tar czf "$SNAP" -C sim/robot arena; then
    "$MJX_PY" -c "$LP; _ledger.mark($PHASE,'verified','$SNAP')"
